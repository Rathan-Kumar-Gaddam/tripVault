...
export default function Dashboard() {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 lg:p-10 pb-28 sm:pb-24">
      ...
      <header className="flex justify-between items-center mb-6 sm:mb-8">
        <div className="min-w-0 pr-3">
          <div className="flex items-center gap-2 mb-1">
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight truncate font-heading">
              {currentTrip.name}
            </h1>
            {isAdmin && (
              <span className="bg-indigo-50 text-indigo-700 border border-indigo-200 text-[10px] sm:text-xs font-extrabold px-2.5 py-0.5 rounded-full shrink-0">
                ADMIN
              </span>
            )}
          </div>
          <p className="text-slate-500 text-xs sm:text-sm font-semibold flex items-center flex-wrap gap-x-2.5 gap-y-0.5">
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          <button
            ...
            className="w-11 h-11 rounded-2xl bg-white text-indigo-600 border border-slate-200 hover:bg-indigo-50 shadow-sm flex items-center justify-center active:scale-90 transition-all"
          ...
          <button
            ...
            className="w-11 h-11 rounded-2xl bg-white text-slate-600 border border-slate-200 hover:bg-slate-50 shadow-sm flex items-center justify-center active:scale-90 transition-all disabled:opacity-50"
          ...
          <button 
            ...
            className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-tr from-indigo-500 via-purple-500 to-pink-500 text-white flex items-center justify-center font-bold text-base overflow-hidden shadow-lg shadow-indigo-500/20 active:scale-95 transition-transform border-2 border-white"
            title="My Profile"
          >
            {user?.avatar ? (
      ...
      {/* ===================== TOP RESPONSIVE HERO & ACTIONS GRID ===================== */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8">
        
        {/* Left Column (Hero Standings + Budget Tracker) */}
        <div className="lg:col-span-7 flex flex-col gap-6">
          ...
          <div className={`relative overflow-hidden p-6 sm:p-7 rounded-[2.5rem] shadow-2xl text-white transition-all ${
            ...
            <WalletCards className="absolute -right-8 -top-8 w-48 h-48 opacity-10 rotate-12 pointer-events-none" />
            
            <div className="flex justify-between items-center mb-3">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-full overflow-hidden border border-white/40 bg-white/20 flex items-center justify-center text-xs font-extrabold shrink-0">
                  {user?.avatar ? (
                    <img src={user.avatar} alt={user.name} className="w-full h-full object-cover" />
                  ) : (
                ...
                <p className="text-white/80 font-bold tracking-wider text-xs uppercase">My Net Standing</p>
              </div>

              <div className="bg-white/15 backdrop-blur-md px-3 py-1 rounded-xl text-xs font-bold border border-white/20 flex items-center gap-1">
            ...
            <div className="grid grid-cols-2 gap-3 mt-5 pt-4 border-t border-white/15 text-xs">
              <div className="bg-black/10 rounded-2xl p-3 flex flex-col">
                <span className="text-[10px] text-white/70 font-semibold uppercase">Owed to You</span>
                <span className="font-extrabold text-emerald-300 text-base">{currency}{totalYouAreOwed.toFixed(2)}</span>
              </div>
              <div className="bg-black/10 rounded-2xl p-3 flex flex-col">
                <span className="text-[10px] text-white/70 font-semibold uppercase">You Owe</span>
                <span className="font-extrabold text-rose-300 text-base">{currency}{totalYouOwe.toFixed(2)}</span>
              </div>
            </div>
            ...
            <div className="mt-3 pt-2 border-t border-white/10 flex justify-end">
              <button
                ...
                className="w-full py-2.5 px-3 bg-white/20 hover:bg-white/30 active:scale-95 text-white font-bold text-xs rounded-xl backdrop-blur-md transition-all flex items-center justify-center gap-1.5 shadow-sm border border-white/20"
              >
          ...
          <div className="bg-white border border-slate-200/80 rounded-[2.5rem] p-6 shadow-sm">
            <div className="flex justify-between items-center mb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-bold text-xs sm:text-sm text-slate-900 uppercase tracking-wider">Trip Budget</h3>
                  <p className="text-[10px] sm:text-xs text-slate-400 font-semibold">Spending limit & target</p>
                </div>
              </div>

              <button
                onClick={() => setShowBudgetModal(true)}
                className="px-3 py-1.5 text-xs font-bold text-indigo-600 hover:text-indigo-700 bg-indigo-50 border border-indigo-100 rounded-xl flex items-center gap-1 active:scale-95 transition-all"
              >
                <Edit3 size={12} />
        ...
        {/* Right Column: Approvals Banners + Quick Action Grid */}
        <div className="lg:col-span-5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-2 gap-3">
            <button
              ...
              className="bg-white border border-slate-200/80 p-4 rounded-2xl flex items-center gap-3.5 shadow-sm hover:border-indigo-400 hover:shadow-md transition-all active:scale-95 group text-left"
            >
              ...
              <div>
                <span className="text-xs font-black text-slate-800 block">Add Expense</span>
                <span className="text-[10px] text-slate-400 font-medium">Split with group</span>
              </div>
            </button>

            <button
              onClick={() => setShowAskModal(true)}
              className="bg-amber-50/70 border border-amber-200/90 p-4 rounded-2xl flex items-center gap-3.5 shadow-sm hover:border-amber-400 hover:shadow-md transition-all active:scale-95 group ring-2 ring-amber-500/10 text-left"
            >
              ...
              <div>
                <span className="text-xs font-black text-amber-900 block">Ask Funds</span>
                <span className="text-[10px] text-amber-700/80 font-medium">Request cover</span>
              </div>
            </button>

            <button
              onClick={() => navigate(`/trip/${id}/add-money?mode=settle`)}
              className="bg-white border border-slate-200/80 p-4 rounded-2xl flex items-center gap-3.5 shadow-sm hover:border-emerald-400 hover:shadow-md transition-all active:scale-95 group text-left"
            >
              ...
              <div>
                <span className="text-xs font-black text-slate-800 block">Settle Up</span>
                <span className="text-[10px] text-slate-400 font-medium">Pay debts</span>
              </div>
            </button>

            <button
              onClick={() => navigate(`/trip/${id}/history`)}
              className="bg-white border border-slate-200/80 p-4 rounded-2xl flex items-center gap-3.5 shadow-sm hover:border-purple-400 hover:shadow-md transition-all active:scale-95 group text-left"
            >
              ...
              <div>
                <span className="text-xs font-black text-slate-800 block">Timeline</span>
                <span className="text-[10px] text-slate-400 font-medium">All activity</span>
              </div>
            </button>
          </div>
      ...
      {/* ===================== MID RESPONSIVE GRID: CATEGORY SPEND & PASSBOOK ===================== */}
        ...
        {sortedCategories.length > 0 && (
          <section className="bg-white p-5 sm:p-6 rounded-[2.5rem] border border-slate-200/80 shadow-sm flex flex-col justify-between">
            <div>
              ...
              <div className="w-full h-3.5 bg-slate-100 rounded-full overflow-hidden flex gap-0.5 p-0.5 border border-slate-200/60 mb-4 shadow-inner">
                ...
                {sortedCategories.map((cat) => {
                  ...
                  return (
                    <div
                      ...
                      className="bg-slate-50/80 p-3.5 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between cursor-pointer hover:bg-indigo-50/50 hover:border-indigo-300 hover:shadow-md active:scale-95 transition-all group"
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-[10px] font-extrabold text-indigo-700 bg-white border border-indigo-100 px-2 py-0.5 rounded-full">
                          {pct}%
                        </span>
                      </div>
                      <div>
      ...
      {/* ===================== BOTTOM RESPONSIVE GRID: RECENT ACTIVITY & GROUP STANDINGS ===================== */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        
        {/* Left: Recent Activity (Clickable Details) */}
        <section className="bg-white p-5 sm:p-6 rounded-[2.5rem] border border-slate-200/80 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-3.5">
              <div>
                      ...
                      <div className="text-right shrink-0">
                        ...
                        {!isSettlement && (
                          <p className={`text-[10px] font-bold mt-0.5 ${
                            isPayer ? 'text-emerald-600' : (myShare > 0 ? 'text-rose-600' : 'text-slate-400')
                          }`}>
                            {isPayer ? `You lent ${currency}${(tx.amount - myShare).toFixed(2)}` : (myShare > 0 ? `Your share ${currency}${myShare.toFixed(2)}` : 'Not involved')}
                          </p>
                        )}
                      </div>
                    </div>
            ...
            <div className="flex flex-col gap-2.5">
              {members.map((member) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-indigo-500 to-purple-500 text-white flex items-center justify-center text-xs font-bold overflow-hidden shrink-0 shadow-sm">
                        {mUser?.avatar ? (
      ...
      <section className="mb-8">
        <div className="bg-gradient-to-br from-slate-900 to-indigo-950 text-white p-6 sm:p-7 rounded-[2.5rem] shadow-xl border border-slate-800">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-3 bg-white/10 rounded-2xl">
              <Download size={20} className="text-indigo-300" />
            </div>
            <div>
              <h3 className="font-extrabold text-sm sm:text-base text-white uppercase tracking-wider font-heading">Export Passbook & Reports</h3>
              <p className="text-xs text-slate-300">Download for Excel/Sheets or generate clean PDF statements</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
            <button
              ...
              className="py-3 px-3 bg-white/10 hover:bg-white/20 active:scale-95 text-white font-bold text-xs rounded-2xl transition-all border border-white/15 flex items-center justify-center gap-2 shadow-sm"
            ...
            <button
              ...
              className="py-3 px-3 bg-indigo-600/70 hover:bg-indigo-600 active:scale-95 text-white font-bold text-xs rounded-2xl transition-all border border-indigo-400/30 flex items-center justify-center gap-2 shadow-sm"
            ...
            <button
              ...
              className="py-3 px-3 bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white font-bold text-xs rounded-2xl transition-all shadow-md shadow-indigo-600/30 flex items-center justify-center gap-2"
